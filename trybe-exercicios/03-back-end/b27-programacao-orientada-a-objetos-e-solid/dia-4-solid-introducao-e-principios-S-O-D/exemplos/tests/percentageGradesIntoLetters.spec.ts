...
describe('Testando a função "percentageGradesIntoLetters"', function () {
  describe('quando é passado um array de disciplinas válidas', function () {
    ...
    const studentResultDisciplines = percentageGradesIntoLetters({ name: 'name', disciplines, school: { name: 'name', approvalGrade: 7.0 }});

    const expectedGrades = ['A', 'B', 'C', 'D', 'E', 'F'];
    const givenGrades = [...studentResultDisciplines.disciplines.map(discipline => discipline.letterGrade)];
